...
for (let index = 0; index < questionsHawaii.length; index++) {
  // display questions
  let questionDisplay = questionsHawaii[index].title;
  console.log(questionDisplay);
  $("#question").text(questionDisplay);

  // display answers
  let answersDisplay = questionsHawaii[index].choices;
  console.log(answersDisplay);
  for (let index = 0; index < answersDisplay.length; index++) {
    function choicesArray () {
      console.log(answersDisplay[3]);
      var choiceButton = $("<button>").attr("id", "choiceButton");
      $("#choiceButton").text(answersDisplay[index]);
      $("#answersList").append(choiceButton);
    }
    choicesArray();
    };
};
